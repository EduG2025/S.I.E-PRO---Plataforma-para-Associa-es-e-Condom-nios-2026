...
/**
 * S.I.E USER MODAL PRO V15.5
 * Protocolo SRE: Gestão Centralizada de Identidade e Auditoria de Ledger.
 * Blindagem contra syntax corruption e duplicação de CPF.
 */
            ...
            <div className="sie-modal-container max-w-6xl h-[92vh]">
                            ...
                            <form className="space-y-12 animate-fade-in">
                                ...
                                <div className="grid grid-cols-1 md:grid-cols-2 gap-10">
                                    <div className="space-y-2">
                                        ...
                                        <input className="w-full h-14 bg-white border border-slate-200 rounded-xl px-6 text-base font-black uppercase outline-none focus:border-indigo-500 shadow-sm" value={formData.name || ''} onChange={e => setFormData({...formData, name: e.target.value})} />
                                    </div>
                                    <div className="space-y-2">
                                        <label className="text-[10px] font-black text-slate-400 uppercase tracking-widest ml-1">CPF (Identificador Único)</label>
                                        <input className="w-full h-14 bg-slate-100 border border-slate-200 rounded-xl px-6 text-base font-mono font-black" value={formData.cpf_cnpj || ''} onChange={e => setFormData({...formData, cpf_cnpj: formatCPF(e.target.value)})} />
                                    </div>
                                    <div className="space-y-2">
                                        <label className="text-[10px] font-black text-slate-400 uppercase tracking-widest ml-1">E-mail Corporativo</label>
                                        <input className="w-full h-14 bg-white border border-slate-200 rounded-xl px-6 text-base outline-none focus:border-indigo-500 shadow-sm" value={formData.email || ''} onChange={e => setFormData({...formData, email: e.target.value})} />
                                    </div>
                                    <div className="space-y-2">
                                        <label className="text-[10px] font-black text-slate-400 uppercase tracking-widest ml-1">Unidade / Lote</label>
                                        <input className="w-full h-14 bg-indigo-50 border border-indigo-100 text-indigo-700 rounded-xl px-6 text-base font-black uppercase outline-none focus:bg-white shadow-inner" value={formData.unit || ''} onChange={e => setFormData({...formData, unit: e.target.value})} />
                                    </div>
                                </div>
                                ...
                                <div className="bg-white border border-slate-200 rounded-[2.5rem] overflow-hidden shadow-sm">
                                    <table className="w-full text-left border-separate border-spacing-0">
                                        <thead className="bg-slate-50 text-[9px] font-black text-slate-400 uppercase tracking-widest border-b">
                                            <tr>
                                                <th className="p-6">Título / Ledger</th>
                                            ...
                                            {userInvoices.map(inv => (
                                                <tr key={inv.id} className="hover:bg-slate-50 transition-colors">
                                                    ...
                                                    <td className="p-6 text-center">
                                                        <span className={`px-4 py-1.5 rounded-xl text-[8px] font-black uppercase border shadow-sm ${inv.status === 'PAID' ? 'bg-emerald-50 text-emerald-600 border-emerald-100' : 'bg-amber-50 text-amber-600 border-amber-100'}`}>{inv.status}</span>
                                                    </td>
                                                </tr>
                                            ))}
                                            {userInvoices.length === 0 && (
                                                <tr><td colSpan={4} className="p-20 text-center text-slate-300 font-black uppercase text-[10px] tracking-widest italic">Sem lançamentos recentes.</td></tr>
                                            )}
                                        </tbody>
                                    </table>
                                </div>
                                ...
                                <button 
                                    onClick={async () => {
                                        ...
                                        try {
                                            ...
                                            alert("Falha ao gerar dossiê preditivo.");
                                        } finally {
                                            setIsGeneratingDossier(false);
                                        }
                                    }} 
                                    className="w-full py-8 bg-slate-900 text-white rounded-[2rem] font-black text-xs uppercase tracking-widest shadow-xl flex items-center justify-center gap-4 hover:bg-indigo-600 transition-all active:scale-95"
                                >
                                    {isGeneratingDossier ? <Loader2 className="animate-spin" size={24}/> : <Brain size={24}/>} Invocando Gemini Advisor
                                </button>
                                {aiDossier && (
                                    <div className="p-12 bg-white border border-slate-200 shadow-inner rounded-[3.5rem] text-slate-700 leading-relaxed uppercase italic text-lg border-l-[10px] border-l-indigo-600">
                                        <Sparkles size={24} className="text-indigo-600 mb-6"/>
                                        {aiDossier}
                                    </div>
                                )}
                            </div>
                ...
                <footer className="h-16 bg-slate-50 border-t border-slate-200 px-10 flex items-center justify-between shrink-0">
                    <div className="flex items-center gap-3">
                        ...
                        <span className="text-[10px] font-black text-slate-400 uppercase tracking-widest">Protocolo Ledger Sincronizado</span>
                    </div>
                    <div className="flex items-center gap-2">
                        <ShieldCheck size={14} className="text-indigo-600"/>
                        <span className="text-[9px] font-black uppercase text-slate-500">SRE Identity Guard Active</span>
                    </div>
                </footer>
            </div>
        </div>
